...
const getPaginationRange = (currentPage, totalPages) => {
  const delta = 1; // Number of page numbers to display on either side of the current active page
  const range = [];
  const rangeWithDots = [];
  let l;
...
function TableDataPublications({ entities = [], yearRange }) {
  ...
  // Calculate the smart truncated pagination range
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-4 flex-wrap select-none">
            ...
            {paginationRange.map((page, index) => {
              // If the value is the ellipsis string, render text structure instead of button
              if (page === "...") {
                return (
                  <span
                    key={`dots-${index}`}
              ...
              // Otherwise render standard action buttons
              return (
                <button
                  key={`page-${page}`}
